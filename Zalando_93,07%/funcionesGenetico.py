...
# Geneticos
def genetico_CHC(baseDatos, nPoblacion, numeroElite, distanciaHinicial, prueba):
    # BaseDatos: 0 -> Numeros ; 1 -> Ropa; 2 -> PyG_Small; 3 -> PyG

    if baseDatos < 2:
        imagenes = preprocesadoImagenes_MNIST(baseDatos)
    else:
        imagenes = preprocesadoImagenesPyG(baseDatos)
    if prueba:
        imagenes = generaTabla() #Las imagenes la pongo como tabla
    #Genero poblacion
    poblacionInicial = nPoblacion
    [poblacion, puntuacion, evaluaciones] = gP.generaPoblacion(poblacionInicial, baseDatos=baseDatos, imagenes=imagenes)
    ...
    while generacion < generacionesMaximas:
        ...
        for i in range(0,len(poblacion)-1,2):
            ...
            if distanciaPadres > distHmax:
                ...
                puntHijo1 = puntuacionCromosoma(hijo1, baseDatos=baseDatos, imagenesPasadas=imagenes)
                if puntHijo1 > 0:
                    evaluaciones += 1
                puntHijo2 = puntuacionCromosoma(hijo2, baseDatos=baseDatos, imagenesPasadas=imagenes)
                if puntHijo2 > 0:
                    evaluaciones += 1
        ...
        if distHmax == 0: # Reinicializo poblacion quedándome con la élite
            ...
            [poblacionNueva, puntuacionNueva, evaluacionesNuevas] = gP.generaPoblacion(nPoblacion - len(poblacionReinicializada), baseDatos=baseDatos,imagenes=imagenes) #Genero el resto de elementos que necesito para la poblacion total
...
# Redes neuronales
def preprocesadoImagenesPyG(baseDatos):
    # Leo las imagenes
    if baseDatos == 2:
        base_dir = '../../../Datos/Archivos/cats_and_dogs_small'
        tamañoBatch = 20
    elif baseDatos == 3:
        base_dir = '../../../Datos/Archivos/cats_and_dogs'
        tamañoBatch = 120
...
def redNeuronal_CNN_PyG(configuracion, train_dir, validation_dir, test_dir):
    if baseDatos == 2:
        tamañoBatch = 40
    else:
        tamañoBatch = 120

    print(f"Len conf: {len(configuracion)}")
    configuracion = traducirConfiguracion(configuracion).copy()
    print(f"Len tras traduc: {len(configuracion)}")

    train_generator = generadores[0]
    validation_generator = generadores[1]
    test_generator = generadores[2]
    ...
    model.add(Conv2D(numero_Filtros, (tamaño_Filtro, tamaño_Filtro), padding='same', strides=tamStrides,
                     activation=funcion_Act,
                     input_shape=(28, 28, 1)))

    if vectorPosicionesPooling.__contains__(1):
    ...
    configuracionConvRest = configuracion[inicioConv + 4:(inicioConv + 4) + (capasConv * 4) + 1]
    print(f"Conf restante: {configuracionConvRest}")
    ...
    for i in range(0, len(configuracionConvRest), 4):
        ...
        model.add(Conv2D(numero_Filtros, (tamaño_Filtro, tamaño_Filtro), padding='same', strides=tamStrides,
                         activation=funcion_Act))
        # Añado capa pooling
        if vectorPosicionesPooling.__contains__(i):
            model.add(MaxPooling2D((2, 2)))
        if valorBatch > 0:
            model.add(BatchNormalization())
        if valorDropout > 0:
    ...
    confCapasDensas = configuracion[inicioDensas:inicioDensas + (capasDens * 4) + 1]
    ...
    model.add(Dense(1, activation='sigmoid')) #Tengo que mantener la capa final binaria

    model.summary()

    moptimizador = configuracion[-2]
    ...
    # Evaluo el test y obtengo resultados
    test_lost, test_acc = model.evaluate(test_generator)
    print("Test Accuracy:", test_acc)

    return test_acc, test_lost


def preprocesadoImagenes_MNIST(baseDatos):
    # Imagenes
    if baseDatos == 0:  # Numeros
...
def redNeuronal_CNN_ropa_numeros(configuracion, baseDatos, imagenes):
    ...
    # Imagenes
    if baseDatos == 0:  # Numeros
        fashion_mnist = mnist = tf.keras.datasets.mnist
    ...
    model.compile(loss='categorical_crossentropy',
                  ...
                  metrics=['accuracy'])
    print("Training")
    model.fit(train_images, train_labels, batch_size=150, epochs=5, verbose=1)
    print("Test")
...
def cruce_CHC_Alternativo(padre1, padre2):
    ...
    capasHijo1,capasHijo2 = cruce(capasHijo1,capasHijo2) #TODO añadirla a los vectores
...
def traducirConfiguracion(configuracion):
    ...
    numero_capasD = numCapasDensasReal[capasDensas - 1]
...
def puntuacionCromosoma(individuo, baseDatos, imagenesPasadas):
    try:
        if baseDatos <= 1:
            print("\t\tBD: numeros o ropa")
            acc, loss = redNeuronal_CNN_ropa_numeros(individuo,baseDatos=baseDatos, imagenes=imagenesPasadas)
        elif baseDatos > 1:
            print("BD: PyG")
            acc, loss = redNeuronal_CNN_PyG(individuo, baseDatos=baseDatos, generadores=imagenesPasadas)
    except:
        acc = 0
